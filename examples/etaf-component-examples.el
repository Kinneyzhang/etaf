...
(defun etaf-component-example-1-basic ()
  ...
  ;; 定义一个简单的 Logo 组件
  (etaf-etml-define-component simple-logo
    :template '(div :class "logo"
                    (span "🚀 ETAF")))
  
  ;; 定义一个简单的 Divider（分隔线）组件
  (etaf-etml-define-component simple-divider
  ...
  (let* ((template '(div
    ...
    (message "组件定义：")
    (message "  (etaf-etml-define-component simple-logo")
...
(defun etaf-component-example-2-props ()
  ...
  ;; 定义一个 Badge（徽章）组件
  (etaf-etml-define-component badge
    :props '(:text :type)
  ...
  ;; 定义一个 Alert（警告框）组件
  (etaf-etml-define-component alert
  ...
  (etaf-etml-define-component user-avatar
    :props '(:name :size)
...
(defun etaf-component-example-3-slots ()
  ...
  (etaf-etml-define-component card
    :props '(:title :footer)
    :template (lambda (data)
  ...
  ;; 定义一个 Modal（模态框）组件
  (etaf-etml-define-component modal
    :props '(:title :visible)
    :template (lambda (data)
                (let ((title (plist-get data :title))
...
(defun etaf-component-example-4-ref ()
  ...
  (let ((count (etaf-etml-ref 0)))
    (message "创建 ref: (etaf-etml-ref 0)")
    (message "  初始值: %s" (etaf-etml-ref-get count))
    
    ;; 修改值
    (etaf-etml-ref-set count 5)
    (message "  设置后 (set 5): %s" (etaf-etml-ref-get count))
    
    ;; 使用 update 更新
    (etaf-etml-ref-update count (lambda (n) (+ n 10)))
  ...
  (etaf-etml-define-component counter
    ...
    :setup (lambda (props)
             (let* ((initial (or (plist-get props :initial) 0))
                    (count (etaf-etml-ref initial))
                    (increment (lambda ()
                                 (etaf-etml-ref-update count #'1+)))
                    (decrement (lambda ()
                                 (etaf-etml-ref-update count #'1-)))
                    (reset (lambda ()
                             (etaf-etml-ref-set count initial))))
               (list :count count
                     :increment increment
                     :decrement decrement
                     :reset reset)))
  ...
  (message "(etaf-etml-define-component counter")
  (message "  :props '(:initial)")
  (message "  :setup (lambda (props)")
  (message "           (let* ((count (etaf-etml-ref initial))")
  (message "                  (increment (lambda () ...)))")
  (message "             (list :count count :increment increment)))")
  (message "  :template (lambda (data) ...))\n")
  
  ;; 演示组件定义后的状态
  (message "组件已注册: %s" (etaf-etml-component-defined-p 'counter))
  (message "组件已注册: %s\n" (etaf-etml-component-defined-p 'toggle)))
...
(defun etaf-component-example-5-computed ()
  ...
  (let* ((price (etaf-etml-ref 100))
         (quantity (etaf-etml-ref 3))
         (total (etaf-etml-computed
                 (lambda ()
                   (* (etaf-etml-ref-get price)
                      (etaf-etml-ref-get quantity)))))
         (discount-rate (etaf-etml-ref 0.1))
         (final-price (etaf-etml-computed
                       (lambda ()
                         (* (etaf-etml-computed-get total)
                            (- 1 (etaf-etml-ref-get discount-rate)))))))
    
    (message "创建 refs:")
    (message "  price = %s" (etaf-etml-ref-get price))
    (message "  quantity = %s" (etaf-etml-ref-get quantity))
    (message "  discount-rate = %s" (etaf-etml-ref-get discount-rate))
    (message "")
    (message "创建 computed:")
    (message "  total = price * quantity = %s" (etaf-etml-computed-get total))
    (message "  final-price = total * (1 - discount) = %s" (etaf-etml-computed-get final-price))
    (message "")
    
    ;; 修改依赖值
    (etaf-etml-ref-set quantity 5)
    (message "修改 quantity = 5 后:")
    (message "  total = %s" (etaf-etml-computed-get total))
    (message "  final-price = %s" (etaf-etml-computed-get final-price))
    (message ""))
  
  ;; 定义一个 PriceCalculator（价格计算器）组件
  (etaf-etml-define-component price-calculator
    :props '(:base-price)
    :setup (lambda (props)
             (let* ((base-price (or (plist-get props :base-price) 0))
                    (quantity (etaf-etml-ref 1))
                    (tax-rate (etaf-etml-ref 0.08))
                    (subtotal (etaf-etml-computed
                               (lambda ()
                                 (* base-price (etaf-etml-ref-get quantity)))))
                    (tax (etaf-etml-computed
                          (lambda ()
                            (* (etaf-etml-computed-get subtotal)
                               (etaf-etml-ref-get tax-rate)))))
                    (total (etaf-etml-computed
                            (lambda ()
                              (+ (etaf-etml-computed-get subtotal)
                                 (etaf-etml-computed-get tax))))))
    ...
    :template (lambda (data)
                `(div :class "price-calculator"
                      ...
                      (div "数量: " ,(format "%d" (etaf-etml-ref-get (plist-get data :quantity))))
                      (div "小计: ¥" ,(format "%.2f" (etaf-etml-computed-get (plist-get data :subtotal))))
                      (div "税费 (" ,(format "%.0f%%" (* 100 (etaf-etml-ref-get (plist-get data :tax-rate))))
                           "): ¥" ,(format "%.2f" (etaf-etml-computed-get (plist-get data :tax))))
                      (div :style "font-weight: bold; margin-top: 8px"
                           "总计: ¥" ,(format "%.2f" (etaf-etml-computed-get (plist-get data :total)))))))
  
  ;; 定义一个 SearchFilter（搜索过滤器）组件
  (etaf-etml-define-component search-filter
    :props '(:items)
    :setup (lambda (props)
             (let* ((items (or (plist-get props :items) '()))
                    (search-term (etaf-etml-ref ""))
                    (filtered-items (etaf-etml-computed
                                     (lambda ()
                                       (let ((term (etaf-etml-ref-get search-term)))
                                         (if (string-empty-p term)
                                           ...
                                           (seq-filter
                                            ...
                                            items))))))
                    (result-count (etaf-etml-computed
                                   (lambda ()
                                     (length (etaf-etml-computed-get filtered-items))))))
               (list :items items
    ...
    :template (lambda (data)
                `(div :class "search-filter"
                      (div "搜索词: " ,(etaf-etml-ref-get (plist-get data :search-term)))
                      (div "匹配数: " ,(format "%d" (etaf-etml-computed-get (plist-get data :result-count))))
                      (ul
                       ,@(mapcar (lambda (item) `(li ,item))
                                 (etaf-etml-computed-get (plist-get data :filtered-items)))))))
  ...
  (message "")
  (message "组件已注册: %s\n" (etaf-etml-component-defined-p 'price-calculator)))
...
(defun etaf-component-example-6-watch ()
  ...
  (let* ((changes nil)
    ...
    (message "  (etaf-etml-watch-source count callback)")
    (message "")
  ...
  (let* ((logs nil)
         (name (etaf-etml-ref "Alice"))
         (age (etaf-etml-ref 25))
         (stop (etaf-etml-watch-effect
                (lambda ()
                  (push (format "用户: %s, 年龄: %s"
                                (etaf-etml-ref-get name)
                                (etaf-etml-ref-get age))
                        logs)))))
    
    (message "创建 watch-effect (自动追踪 name 和 age):")
    (message "  初始运行: %s" (car logs))
    
    (etaf-etml-ref-set name "Bob")
    (message "  修改 name: %s" (car logs))
    
    (etaf-etml-ref-set age 30)
  ...
  (etaf-etml-define-component auto-save-demo
    :props '(:initial-content)
    :setup (lambda (props)
             (let* ((content (etaf-etml-ref (or (plist-get props :initial-content) "")))
                    (save-status (etaf-etml-ref "已保存"))
                    (last-saved (etaf-etml-ref nil))
                    ;; 模拟自动保存
                    (save-fn (lambda ()
                               (etaf-etml-ref-set save-status "保存中...")
                               ;; 模拟异步保存
                               (etaf-etml-ref-set last-saved (format-time-string "%H:%M:%S"))
                               (etaf-etml-ref-set save-status "已保存"))))
               ;; 监听内容变化
    ...
    :template (lambda (data)
                `(div :class "auto-save"
                      (div "内容: " ,(etaf-etml-ref-get (plist-get data :content)))
                      (div :style "color: #666; font-size: 12px"
                           "状态: " ,(etaf-etml-ref-get (plist-get data :save-status))
                           ,@(when (etaf-etml-ref-get (plist-get data :last-saved))
                               `(" | 上次保存: " ,(etaf-etml-ref-get (plist-get data :last-saved))))))))
  
  (message "--- AutoSave 组件概念 ---")
  (message "使用 watch-source 监听内容变化并自动保存")
  (message "组件已注册: %s\n" (etaf-etml-component-defined-p 'auto-save-demo)))
...
(defun etaf-component-example-7-todo-list ()
  ...
  ;; 首先定义一个 TodoItem 组件
  (etaf-etml-define-component todo-item
  ...
  ;; 定义 TodoList 主组件
  (etaf-etml-define-component todo-list
    :props '(:initial-items)
    :setup (lambda (props)
             (let* ((initial (or (plist-get props :initial-items) '()))
                    (items (etaf-etml-ref initial))
                    (new-item-text (etaf-etml-ref ""))
                    (filter-type (etaf-etml-ref 'all)) ; 'all, 'active, 'completed
                    ;; 计算属性
                    (filtered-items (etaf-etml-computed
                                     (lambda ()
                                       (let ((all-items (etaf-etml-ref-get items))
                                             (filter (etaf-etml-ref-get filter-type)))
                                         (pcase filter
                                           ('all all-items)
                                           ('active (seq-filter
                                                     (lambda (item)
                                           ...
                                           ('completed (seq-filter
                                                        ...
                                                        all-items)))))))
                    (total-count (etaf-etml-computed
                                  (lambda ()
                                    (length (etaf-etml-ref-get items)))))
                    (active-count (etaf-etml-computed
                                   (lambda ()
                                     (length (seq-filter
                                              (lambda (item)
                                                (not (plist-get item :completed)))
                                              (etaf-etml-ref-get items))))))
                    (completed-count (etaf-etml-computed
                                      (lambda ()
                                        (- (etaf-etml-computed-get total-count)
                                           (etaf-etml-computed-get active-count)))))
                    ;; 方法
                    (add-item (lambda ()
                                (let ((text (etaf-etml-ref-get new-item-text)))
                                  (when (not (string-empty-p text))
                                    (let* ((current (etaf-etml-ref-get items))
                                           (new-id (1+ (or (seq-max (mapcar (lambda (i) (plist-get i :id)) current)) 0)))
                                           (new-item (list :id new-id :text text :completed nil)))
                                      (etaf-etml-ref-set items (append current (list new-item)))
                                      (etaf-etml-ref-set new-item-text ""))))))
    ...
    :template (lambda (data)
                (let ((filtered (etaf-etml-computed-get (plist-get data :filtered-items)))
                      (active (etaf-etml-computed-get (plist-get data :active-count)))
                      (completed (etaf-etml-computed-get (plist-get data :completed-count))))
                  `(div :class "todo-app"
  ...
  (message "")
  (message "组件已注册:")
  (message "  todo-item: %s" (etaf-etml-component-defined-p 'todo-item))
  (message "  todo-list: %s\n" (etaf-etml-component-defined-p 'todo-list)))
...
(defun etaf-component-example-8-form ()
  ...
  ;; 定义 Form（表单）组件
  (etaf-etml-define-component form
    :props '(:title :on-submit)
    :setup (lambda (props)
             (let* ((form-data (etaf-etml-reactive '(:name "" :email "" :role "")))
                    (errors (etaf-etml-ref '()))
                    (submitting (etaf-etml-ref nil))
                    (validate (lambda ()
                                (let ((errs '()))
                                  (when (string-empty-p (etaf-etml-reactive-get form-data :name))
                                    (push '(:name . "姓名不能为空") errs))
                                  (when (string-empty-p (etaf-etml-reactive-get form-data :email))
                                    (push '(:email . "邮箱不能为空") errs))
                                  (etaf-etml-ref-set errors errs)
                                  (null errs))))
                    (submit (lambda ()
                              (when (funcall validate)
                                (etaf-etml-ref-set submitting t)
                                ;; 模拟提交
                                (message "提交表单: %S" (etaf-etml-reactive-to-plist form-data))
                                (etaf-etml-ref-set submitting nil)))))
    ...
    :template (lambda (data)
                (let ((title (plist-get data :title))
                      (errors (etaf-etml-ref-get (plist-get data :errors)))
                      (submitting (etaf-etml-ref-get (plist-get data :submitting))))
                  `(div :class "form"
  ...
  (message "")
  (message "组件已注册:")
  (message "  form-field: %s" (etaf-etml-component-defined-p 'form-field))
  (message "  form-select: %s" (etaf-etml-component-defined-p 'form-select))
  (message "  form: %s\n" (etaf-etml-component-defined-p 'form)))
...
(defun etaf-component-run-all-examples ()
  ...
  ;; 列出所有已注册的组件
  (message "已注册的组件列表:")
  (dolist (comp (etaf-etml-component-list-all))
    (message "  - %s" comp)))
