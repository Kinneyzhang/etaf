...
(should-equal
 (etaf-template-render '(div "Hello, {{ name }}!") '(:name "World"))
 '(div "Hello, World!"))

(should-equal
 (etaf-template-render '(div "Count: {{ count }}") '(:count 42))
 '(div "Count: 42"))

(should-equal
 (etaf-template-render '(p "{{ greeting }}, {{ name }}!")
                       '(:greeting "Hi" :name "Alice"))
 '(p "Hi, Alice!"))

;; Test nested property access
(should-equal
 (etaf-template-render '(div "{{ user.name }}") '(:user (:name "Bob")))
 '(div "Bob"))

;; Test empty interpolation
(should-equal
 (etaf-template-render '(div "Value: {{ missing }}") '(:other "x"))
 '(div "Value: "))

;;; v-if Conditional Rendering Tests

(should-equal
 (etaf-template-render '(div (p :v-if "visible" "Visible")) '(:visible t))
 '(div (p "Visible")))

(should-equal
 (etaf-template-render '(div (p :v-if "visible" "Visible")) '(:visible nil))
 '(div))

(should-equal
 (etaf-template-render '(div (p :v-if "count" "Has count")) '(:count 5))
 '(div (p "Has count")))

(should-equal
 (etaf-template-render '(div (p :v-if "count" "Has count")) '(:count 0))
 '(div))

;; Test negation
(should-equal
 (etaf-template-render '(div (p :v-if "!hidden" "Shown")) '(:hidden nil))
 '(div (p "Shown")))

(should-equal
 (etaf-template-render '(div (p :v-if "!hidden" "Shown")) '(:hidden t))
 '(div))

;;; v-if/v-else Tests

(should-equal
 (etaf-template-render
  '(div
...
(should-equal
 (etaf-template-render '(div (p :v-show "visible" "Text")) '(:visible t))
 '(div (p "Text")))

(should-equal
 (etaf-template-render '(div (p :v-show "visible" "Text")) '(:visible nil))
...
(should-equal
 (etaf-template-render '(div (p :v-text "message")) '(:message "Hello"))
 '(div (p "Hello")))

(should-equal
 (etaf-template-render '(div (p :v-text "count")) '(:count 99))
 '(div (p "99")))
...
;; Boolean literals
(should-equal
 (etaf-template-render '(div (p :v-if "true" "Always")) '())
 '(div (p "Always")))

(should-equal
 (etaf-template-render '(div (p :v-if "false" "Never")) '())
 '(div))

;; Number literals in expressions
(should-equal
 (etaf-template-render '(div "{{ '42' }}") '())
 '(div "42"))

;;; Attribute Interpolation Tests

(should-equal
 (etaf-template-render '(a :href "/user/{{ id }}" "Link") '(:id 123))
 '(a :href "/user/123" "Link"))

(should-equal
 (etaf-template-render '(div :class "item-{{ type }}" "Content") '(:type "active"))
 '(div :class "item-active" "Content"))

;;; Reactive Data Tests

(let* ((reactive (etaf-template-create-reactive '(:name "Alice" :count 0)))
       (changes nil))
  ;; Test get
  (should-equal (etaf-template-get reactive :name) "Alice")
  (should-equal (etaf-template-get reactive :count) 0)
  
  ;; Test watch and set
  (etaf-template-watch reactive
    (lambda (r k v) (push (list k v) changes)))
  (etaf-template-set reactive :count 5)
  (should-equal (etaf-template-get reactive :count) 5)
