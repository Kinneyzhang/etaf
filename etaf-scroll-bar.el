...
(defun etaf-scroll-bar-pixel (scroll-bar &optional border-box-p)
...
(defun etaf-scroll-bar-track-face (scroll-bar)
  "滚动条上下的轨道部分的face"
...
(defun etaf-scroll-bar-thumb-face (scroll-bar idx)
...
(defun etaf-scroll-bar-render (scroll-bar box-uuid scroll-steps)
    ...
    (let* ((track-color (oref scroll-bar track-color))
           ...
           (basic-track-str
            (propertize
             ...
             'face (etaf-scroll-bar-track-face scroll-bar)))
           ;; 在 track-height 的滚动条中，标记出实际的滚动条并设置属性：
           ;; 1. 给滚动条首行设置 etaf-v-scroll-bar-start: box-uuid
           ;; 2. 给滚动条尾行设置 etaf-v-scroll-bar-end: box-uuid
            ...
            (with-temp-buffer
              ...
              (dotimes (idx thumb-height)
                (add-text-properties
                 ...
                 `( face ,(etaf-scroll-bar-thumb-face scroll-bar idx)
                    ,@(when (= idx 0)
                        `( etaf-v-scroll-thumb-head ,box-uuid
                           ;; etaf-v-scroll-steps ,scroll-steps
                           ))
                    ,@(when (= idx (1- thumb-height))
                        `(etaf-v-scroll-thumb-tail ,box-uuid))))
                (forward-line 1))
              (buffer-string))))
      ;; 使用 etaf-v-scroll-thumb-area 属性标记整个滚动条的可活动范围
      (setq track-height-thumb-str
            (propertize track-height-thumb-str
                        'etaf-v-scroll-area box-uuid))
      (etaf-box-string
       (etaf-box
